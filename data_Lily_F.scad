...
part=0; //[0:None -Library-, 1:Elevated foot (Fisher rods), 2:Flat foot (rods down to floor), 3:Wall support (for non elevated foot), 4:Motor_support, 5:Carriage, 6:Tensioner for plain bearing, 7:Tensioner for flanged bearing,8:Tensioner wheels, 9:Arm junctions, 10:Extruder bracket, 11:Board support, 12:Glass lock, 14:Power brick retainer, 15:Lily Name plate, 16:Screw hiding plates, 17:Door opening pad, 19:Reference stick (length to adjust structure triangle), 50:Panel0, 51:Panel1, 52:Panel2,53:Panel3, 54:Panel4, 55:Panel5,56:Panel6, 57:Panel7, 58:Panel8, 67:Panel 17]
xpart=0;
motor_offset_a = 0;
...
/*[Dataset]*/
htop = 0;  // height of top structure
...
move_rot  = -30;
//Text
txtzpos = 360; // position of the message panel top
txtangle=-90;
...
Retainer_STL = "STL/Lily_33_effector_retainer.stl";
Cable_retainer_STL = 
  "STL/Lily_32_cable_retainer.stl";
...
//Motor support height (46 for the Lily F)
mot_support_height = 46;
top_stop_thickness = 5;
top_clearance =  top_stop_thickness + mot_support_height; // motor 43 // clearance between top of the carriage and top structure - 5 is height of top pads, 46 is height of motor support
...
/*/Initial dataset - my own machine see next dataset
...
htotal=520;
bed_level = 20; //Support + bed heater +bed
beam_int_radius = 139;
arm_length = 204;
mini_angle = 23; 
basewd = 380;
...
basedpoffset = 6;
$wallsup = 0;
//Rod base shall take into account the bed level
rod_base  = 66;
$ht_tens = 92;
hotend_vert_dist = 31;
$bedDia=200; 
...
if (part) { // set part =0 (no part) for delta simulation
  ...
  if (part==1) foot(); //foot with elevated support, for Fisher std rods
    ...
    motor_support(); 
    *t(0,motor_offset)rot(-90)nema17();
  }  
  ...
  else if (part==10) dmirrorx() t(28) rot (90-22.5) extruder_bracket(); // extruder plate attach 
  else if (part==11) { 
    boardsup();
...
module buildSides() {
  // motor support 
  rot120() t(0,beam_int_radius,htotal) {
    t(0,posrod) // positioning rods
      rotz(30) {
        gray() { 
  ...
  if ($boxed) // filter  - before panel -> transparency
    rotz (30) 
      t(platedist+boxpanelthk, 0, htotal-175)  
        black() if ($isAngle) cubex(15,230,330);
  color (panel_color) {
    for (i=[30, 120+30])
      rotz(i)  // structural front panels - 1 -
        t(-splatedist-panelthk/2) panel(1);
          
    rotz(-90) // structural back panel - 2 -
      t(-splatedist-panelthk/2) panel(2);
    if ($boxed) {  // if the printer is enclosed.
      
      rotz (-30) //closing side panel left - 3 -
        t(-platedist-boxpanelthk/2, -dpshift/2) //closing side panel left - 3 -
        panel(3);
      rotz (30) 
        t(platedist+boxpanelthk/2, -dpshift/2)  //closing side panel right with filtration holes -4 -
          panel(4);
    
      mirrorx()  // side doors left - 5 - right - 6 
        t((basewd-30)/2+boxpanelthk/2, splatedist+panelthk- sd_doorwd/2)
          panel(5);
      t((basewd-30)/2+boxpanelthk/2, splatedist+panelthk- sd_doorwd/2)
          panel(6);
      t(0,-platedist-panelthk/2-boxpanelthk/2) //front  door -6 -
        rotz(90) panel(7);
    } // boxed 
    if (htsub) {
      dmirrorx()   {
...
module panel (num){
  ...
  if (num==1) { 
    ...
    diff() {
      ...
      //::::::::::::::::::::::
      duplz (450,0) t(0,0,windoorht/2+30) // glass hole
    ...
    diff() {
      ...
      dmirrorx() 
        rotz(30)  // structural front panels cut
          t(-splatedist-panelthk) cubex (-200, 600, 50);
      rotz (-30)  // cooling hole   
        t(-platedist-boxpanelthk-20, -15) 
          hull() 
...
module buildAllFrame() {
  ...
  color(struct_color) {
    rot120() t(0,beam_int_radius,0) {
      if($isAngle) 
        rotz(90) tslz(0.01) angleSup(); 
      else if ($wallsup) {
        foot2();
         t(0,0,$wallsup) wallsup();
      }
      else foot(); // foot for rods 
      if(!$noTopSup)
        color(struct_color) 
          tslz(htotal+motor_voffset) 
            motor_support();
     }   
    rot120 (-30)  
      t(beam_int_radius,0,$ht_tens) {
        tensioner(); 
        t(belt_axis,0,-12) twheel();
      }  
  } 
  gray() { // rail 
    rot120(-30) {
      if (!$noTopSup)
        rotz(30) 
          t(0,beam_int_radius+motor_offset,htotal+motor_voffset) // motor
            rot(-90) nema17(32);   
      if (rod_space)
        dmirrory()  //rods
  ...
  if(htsub) { // side extruder if spool in base
    ...
    rotz(-90) 
      t(beam_int_radius-65,25,htotal-125) {
        r(0,90) {
          nema17(32); // ??
          t(0,0,4) 
...
axis_h = htTens/2+1;  
  difference() {
    ...
    t(-11+belt_axis,0,axis_h)
      rot(0,-90) cylinder (d2=6.5, d1=3, h=1.4); 
    t(9.5+belt_axis,0,axis_h)  
...
module buildEffector () {
   ...
   * t(0,0,25) {
      rotz (180) t(0,25,-2)   rot(-90) build_fan(30,6);
      rotz (60)  t(0,22.5,-2) rot(-80) build_fan(30,6);
      rotz (300) t(0,22.5,-2) rot(-80) build_fan(30,6);
    }
  red()
    rotz(120) 
      tslz(24) mirrorz()
        import(Cable_retainer_STL);  
  orange() tslz(32) mirrorz() 
     rotz(210) 
      import(Retainer_STL );  
      //effector_retainer();  
}
  ...
  if ($wallsup)
    color("silver")   {
      t(0,0,topsup){
        ...
        rotz(-75)
          cubez(16,14,8,
              4.5,2.8,-30-topfin-0.5);
      }  
    }
  else {  
  * rotz (90) import("Hexagon-102.stl");
  *  t(0,0,-hotend_vert_dist)
        cone3z (1,4, 0.1,2,50);
  *  t(0,0,-hotend_vert_dist+20)    
      cylz(17,40, 0,0,0, 6);
  }  
}
...
module foot () {
  ...
  difference() { 
    ...
    dmirrorx()  {
      ...
      t(rodrd,7,rodht) // biased cut for triangle rods seats
        rotz(30) {
          cyly(-4,22,  dec, -12);
          t(dec)
            hull() {
...
module carriage () {
  ...
  difference() { 
    ...
    cconez(3.2,2.6, 5,20,
        -sw_offset,9.5,-1);//switch actuation
    cconez(2.5,3.2, -2,20, 
          6,-12.5+belt_axis,21.9); // stop fixation
    dmirrorx()  {
 ...
 * cylz(15,-8, 6,-11); //rubber stop
  dmirrorx() 
    difference() {
       ...
       t(arm_space/2,-car_hor_offset, car_vert_dist) {
         rot (35) {
...
//Unused modules to stop warning 
//They are linked to other printer type (Lily-Big)
module top_renf () {}
module motorPlate () {}
module angleSup (par=false) {}
